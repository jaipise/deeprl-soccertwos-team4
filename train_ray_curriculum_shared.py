...
class CurriculumSharedSelfPlayCallback(DefaultCallbacks):
    def on_train_result(self, **info):
        ...
        if (
            ...
            print("---- Updating shared-policy opponent archive!!! ----")
            trainer.set_weights(
                {
                    "opponent_gen_3": trainer.get_weights(["opponent_gen_2"])[
                        "opponent_gen_2"
                    ],
                    "opponent_gen_2": trainer.get_weights(["opponent_gen_1"])[
                        "opponent_gen_1"
                    ],
                    "opponent_gen_1": trainer.get_weights(["default"])["default"],
                }
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--variant",
        default=os.environ.get("SHAPING_VARIANT", "V5_dynamic_aggressive"),
...
if __name__ == "__main__":
    ...
    print(f"---- Training shared-policy variant: {args.variant} ----")
    analysis = tune.run(
        "PPO",
        name=f"PPO_curriculum_shared_{args.variant}",
        config={
            "num_gpus": 0,
            ...
            "log_level": "INFO",
            "framework": "torch",
            "callbacks": CurriculumSharedSelfPlayCallback,
            "multiagent": {
                "policies": {
                    "default": (None, obs_space, act_space, {}),
                    "opponent_gen_1": (None, obs_space, act_space, {}),
                    "opponent_gen_2": (None, obs_space, act_space, {}),
                    "opponent_gen_3": (None, obs_space, act_space, {}),
                },
